  ...
  return <div className="fixed inset-0 z-[70]" role="dialog" aria-modal="true" aria-label="Record details">
    ...
    <aside className="absolute inset-y-0 right-0 flex w-full max-w-[520px] flex-col border-l border-[#dedee1] bg-white shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        ...
        <section className="mt-7"><h3 className="section-label">Issues</h3><div className="mt-3 space-y-2">{record.issue_reasons.length ? record.issue_reasons.map((issue) => <div key={issue} className="rounded-lg bg-amber-50 px-3 py-2.5 text-xs leading-5 text-amber-900">{issue}</div>) : <p className="text-sm text-[#777]">No unresolved issues.</p>}</div></section>
