  ...
  return <section className="overflow-hidden rounded-xl border border-[#e3e3e5] bg-white">
    ...
    <div className={`overflow-auto ${compact ? 'max-h-[360px]' : 'max-h-[calc(100vh-250px)] min-h-[440px]'}`}>
      <table className="w-full min-w-[1040px] border-collapse text-left text-sm">
        ...
        <tbody className="divide-y divide-[#eeeeef]">{filtered.map((record) => <tr key={record.id} onClick={() => onSelect?.(record)} className={`${onSelect ? 'cursor-pointer' : ''} h-16 transition hover:bg-[#fafafa]`}><td className="max-w-[260px] px-5 py-3"><p className="truncate font-medium text-[#222]">{record.cleaned_product_name || 'Unnamed product'}</p><p className="mt-1 font-mono text-[11px] text-[#999]">{record.sku || 'NO-SKU'}</p></td><td className="px-4 py-3 text-[#666]">{record.cleaned_category || '—'}</td><td className="px-4 py-3 font-medium">{record.cleaned_price != null ? `${record.cleaned_currency ?? ''} ${record.cleaned_price.toFixed(2)}` : '—'}</td><td className="px-4 py-3 text-[#666]">{record.cleaned_inventory ?? '—'}</td><td className="px-4 py-3"><div className="flex items-center gap-2"><div className="h-1.5 w-16 overflow-hidden rounded-full bg-[#e9e9eb]"><div className="h-full rounded-full bg-blue-500" style={{ width: `${record.confidence_score}%` }} /></div><span className="text-xs font-medium">{record.confidence_score.toFixed(0)}%</span></div></td><td className="px-4 py-3"><StatusBadge status={record.status} /></td><td className="px-4 py-3"><ReviewStatusBadge record={record} /></td><td className="px-5 py-3 text-right"><span className={record.issue_reasons.length ? 'font-medium text-red-600' : 'text-[#aaa]'}>{record.issue_reasons.length}</span></td></tr>)}</tbody>
      </table>
